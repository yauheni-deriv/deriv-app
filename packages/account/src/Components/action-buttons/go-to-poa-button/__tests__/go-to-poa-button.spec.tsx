...
describe('<GoToPoaButton/>', () => {
    const history = createBrowserHistory();
    const renderWithRouter = (component: React.ReactElement) => {
        return render(<Router history={history}>{component}</Router>);
    };

    it('should render GoToPoaButton component', () => {
        renderWithRouter(<GoToPoaButton />);
        expect(screen.getByTestId('poa_button_text')).toBeInTheDocument();
    });
    it('should render GoToPoaButton component with default message', () => {
        renderWithRouter(<GoToPoaButton />);
        expect(screen.getByText(/submit proof of address/i)).toBeInTheDocument();
    });

    it('should render custom_text passed to the component', () => {
        renderWithRouter(<GoToPoaButton custom_text={<span>Lorem Ipsum</span>} />);
